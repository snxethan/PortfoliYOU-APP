        ...
        <div className="bg-[color:var(--bg)] border border-[color:var(--border)] rounded-md p-3 mb-4 max-w-xl mx-auto">
            {mode === 'create' ? (
                <div className="flex flex-col sm:flex-row items-start gap-3">
                    <input autoFocus className="input w-full sm:w-80" placeholder="Portfolio name" value={name} onChange={e => setName(e.target.value)} />
                ...
                <div className="flex items-center gap-4 justify-center py-1">
                    <button className="btn btn-outline" onClick={() => setMode('create')}><Plus size={14} className="mr-1" /> Create new Portfolio</button>
